 ...
 */
export async function fetchAllMcps() {
    const allMcps = [];
    let page = 0;
    let totalPages = 1;
    while (page < totalPages) {
        const response = await fetch(`${PLAYMCP_API_BASE}/mcps?page=${page}&sortBy=FEATURED_LEVEL&pageSize=20`, {
            headers: {
 ...
 */
export async function fetchMcpDetail(mcpId) {
    try {
        const response = await fetch(`${PLAYMCP_API_BASE}/mcps/${mcpId}`, {
            headers: {
                'accept': 'application/json',
                'content-type': 'application/json',
            },
 ...
 * MCP 페이지 URL 생성
 */
export function getMcpPageUrl(mcpId) {
    return `https://playmcp.kakao.com/mcp/${mcpId}`;
}
//# sourceMappingURL=playmcpApi.js.map