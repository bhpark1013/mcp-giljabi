...
let cachedMcps: McpData[] | null = null;

/**
 * 캐시 파일에서 데이터를 로드합니다
 */
function loadFromCache(): McpData[] | null {
  try {
 ...
 * 데이터를 캐시 파일에 저장합니다
 */
function saveToCache(mcps: McpData[]): void {
  try {
 ...
 */
export async function initMcpData(): Promise<McpData[]> {
  // 메모리 캐시 확인
  if (cachedMcps) {
    return cachedMcps;
  }
  ...
  const mcpData: McpData[] = mcps.map((mcp) => ({
    id: mcp.id,
    name: mcp.name,
 ...
 * 캐시된 MCP 데이터를 반환합니다
 */
export function getCachedMcps(): McpData[] | null {
  return cachedMcps;
}
 ...
 * 캐시를 강제로 갱신합니다
 */
export async function refreshCache(): Promise<McpData[]> {
  cachedMcps = null;
  const mcps = await fetchAllMcps();
